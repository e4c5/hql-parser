...
public class QueryAnalysisVisitor extends HQLBaseVisitor<Void> {
    ...
    public Void visitSelectItem(SelectItemContext ctx) {
        ...
        // If it starts with NEW keyword, skip the path (class name) and only visit
        // expressionList
    ...
    // The key insight: BaseVisitor's implementation of each visit method returns
    // defaultResult()
    ...
    public Void visitJoinClause(JoinClauseContext ctx) {
        if (ctx.path() != null) {
            // The path in a join clause is like "u.orders" where u is alias, orders is
            // collection field
    ...
    public Void visitPath(PathContext ctx) {
        ...
        if (identifiers.size() == 1) {
            ...
            // Only proceed if this is a known entity or alias
            // If it's unknown, it might be a fully qualified class name (enum, constant,
            // etc.)
            // which we shouldn't treat as an entity reference.
            if (analysis.getEntityNames().contains(entity) || aliasToEntity.containsKey(first)) {
                // Add field to entity
